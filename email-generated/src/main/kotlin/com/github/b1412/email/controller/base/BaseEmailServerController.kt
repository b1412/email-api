...
@Transactional
abstract class BaseEmailServerController : BaseController<EmailServer, Long>() {

@GraphRender("emailServer")
@GetMapping
...
@PostMapping
override fun saveOne(@Validated @RequestBody input: EmailServer, request: HttpServletRequest, uriComponent: UriComponentsBuilder): ResponseEntity<*> {
return super.saveOne(input, request, uriComponent)

}

@PutMapping("{id}")
override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: EmailServer, request: HttpServletRequest): ResponseEntity<*> {
super.updateOne(id, input, request)
return ResponseEntity.noContent().build<EmailServer>()
}
